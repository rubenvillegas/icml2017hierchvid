...
def main(lr, image_size, batch_size, layer, alpha, beta, gamma, steps,
  ...
  updateD = True
  updateG = True
  prefix = ('PENNACTION_ANALOGY_imgsize=' + str(image_size) + '_layer=' +
      ...
      while iters < num_iter:
        ...
        for _, batchidx in mini_batches:
          if len(batchidx) == batch_size:
            ...
            pose_batch = np.zeros(
                (batch_size, image_size, image_size, 2, 48), dtype='float32')
            ...
            output = parallel(
                delayed(load_penn_data)(f, p, img_sze, s)
                for f, p, img_sze, s in zip(tfiles, paths, shapes, tsteps))
